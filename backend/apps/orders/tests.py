...
class OrderModelTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.product = Pattern.objects.create(
            article=123456,
            name="Test Pattern",
            price=123
        )
...
class DataManagerTestCase(TestCase):
    def setUp(self):
        ...
        self.products = [
            Pattern.objects.create(
                article=prod["article"], price=123
    ...
    def test_save_data(self, mock_send_order_data):
        ...
        with patch.object(
                self.manager, 'save_customer', return_value=self.customer) as mock_save_customer, \
                patch.object(
                    self.manager, 'save_order',  return_value=self.order) as mock_save_order, \
                patch.object(
                    self.manager, 'save_products', return_value=self.products) as mock_save_products:
...
class WebhookViewTestCase(TestCase):
    ...
    def test_post_success(self, mock_serializer, mock_send_order_data):
        ...
        mock_serializer.return_value.serialize.return_value = (
            self.serialized_customer, self.serialized_order, self.serialized_product
        )
